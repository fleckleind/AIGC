...
class LPIPS(nn.Module):
    ...
    def load_from_pretrained(self, name="vgg_lpips"):
        ckpt = get_ckpt_path(name, '/kaggle/working/')
        self.load_state_dict(torch.load(ckpt, map_location=torch.device("cpu")), strict=False)

    def forward(self, real_x, fake_x):
        # x->normalization->vgg16->feature map
        features_real = self.vgg(self.scaling_layer(real_x))
        features_fake = self.vgg(self.scaling_layer(fake_x))
        diffs = {}

        for i in range(len(self.channels)):
            # norm_tensor: x / inner-sum-distance
            # different between feature maps with different resolution/layer
            diffs[i] = (norm_tensor(features_real[i]) - norm_tensor(features_fake[i])) ** 2
        # spatial_average: average in height and width (2,3) dimension
